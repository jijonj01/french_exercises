...
class A1Four extends StatefulWidget {
  const A1Four({Key? key}) : super(key: key);
...
class _A1FourState extends State<A1Four> with SingleTickerProviderStateMixin {
  // Primary colors from the original HTML/CSS
  ...
  // Helpers to compute section scores and totals
  int get muetsScore =>
      (q1MuetsCorrect ? 1 : 0) +
      (q2MuetsCorrect ? 1 : 0) +
      (q3MuetsCorrect ? 1 : 0);
  ...
  // --- Handlers for text-input ("Élision") questions ---
  ...
  // Generic builder for multiple-choice questions
  ...
  // Builder for text-input questions
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Jeu Interactif : Astuces du Français'),
