...
/// Main widget for the French Articles Game (Variante 1)
class A1Eight extends StatefulWidget {
  final String imageUrl;
  const A1Eight({Key? key, required this.imageUrl}) : super(key: key);

  @override
  _A1EightState createState() => _A1EightState();
}

class _A1EightState extends State<A1Eight> {
  // ====== Section: Articles Indéfinis (Variante 1) ======
  final List<WordItem> _initialWordsIndefini = [
    WordItem(text: 'chat', genre: 'masculin', isPlural: false),
    WordItem(text: 'table', genre: 'féminin', isPlural: false),
    WordItem(text: 'chiens', genre: 'masculin', isPlural: true),
    WordItem(text: 'fleurs', genre: 'féminin', isPlural: true),
  ];
  ...
  // ====== Section: Articles Définis (Variante 1) ======
  ...
  /// Check if a word matches the indefinite article rules (Variante 1)
  bool _matchesIndefini(String article, WordItem word) {
    ...
    if (article == 'Une' && word.genre == 'féminin' && !word.isPlural)
      return true;
    if (article == 'Des' && word.isPlural) return true;
    return false;
  }

  /// Check if a word matches the definite article rules (Variante 1)
  bool _matchesDefini(String article, WordItem word) {
    ...
    if (article == 'La' && word.genre == 'féminin' && !word.isPlural)
      return true;
    if (article == 'Les' && word.isPlural) return true;
    return false;
  }
      ...
      onAccept: (word) {
        ...
        if (correct) {
          setState(() {
            ...
            // Check if all words have been placed in the appropriate section
            if (isIndefiniSection) {
              ...
              final totalPlacedDef = _placedDefini.values.fold<int>(
                0,
                (sum, list) => sum + list.length,
              );
              if (totalPlacedDef == _initialWordsDefini.length) {
            ...
            SafeArea(
              child: Column(
                children: [
                  ...
                  // Page Title
                  const Text(
                    'Jeu pour Apprendre les Articles Français – Variante 1',
                    style: TextStyle(
                      fontSize: 24,
                      fontWeight: FontWeight.bold,
                  ...
                  // Section 1 : Articles Indéfinis (Variante 1)
                  _buildSection(
                  ...
                  // Section 2 : Articles Définis (Variante 1)
                  _buildSection(
